...
def create_status(
    ...
    data_type: int = 255,
    subtype: int = 17,
    create_status_value: CreateStatus = CreateStatus.OK,
) -> bytes:
    payload = struct.pack(
        "<HBBHBBH",
...
class UploaderTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_upload_decodes_no_space_for_type_create_file_failure(self):
        data = b"\xd0\x00\xd0abc"
        transport = FakeTransport(
            [
                generic_status(GarminMessage.SYSTEM_EVENT),
                create_status(file_index=0xFFFF, create_status_value=CreateStatus.NO_SPACE_FOR_TYPE),
            ]
        )
        uploader = GarminPrgUploader(transport, max_packet_size=32, post_sync_delay=0)

        with self.assertRaisesRegex(ProtocolError, "NO_SPACE_FOR_TYPE"):
            await uploader.upload(data)
